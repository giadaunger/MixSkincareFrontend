      ...
      <div className="md:flex inline">
        <div className="bg-white rounded-xl p-6 md:w-3/4">
          {analysisResult.active_ingredients?.map((active, index) => (
            <div 
              key={index} 
              className="sm:flex w-full items-center gap-3 bg-[#fffcc8] text-[#e3ba5f] font-bold rounded-xl sm:rounded-full mt-4 px-6 py-3 max-w-2xl mx-auto"
            >
          ...
          {analysisResult.incompatibility_warnings?.map((warning, index) => (
            <div 
              key={index}
              className="sm:flex w-full items-center gap-3 bg-[#cef4fa] text-[#7ac4d1] font-bold rounded-xl sm:rounded-full mt-4 px-6 py-3 max-w-2xl mx-auto"
            >
              <Warning className="w-8 h-8 flex-shrink-0" />
              <span className="text-center">
